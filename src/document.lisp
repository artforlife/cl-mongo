...
(defun print-hash.3 (ht stream &key (max (hash-table-count ht)))
  (labels ((prdocs (docs)
	     (format stream "~1,1T{")
	     (block print-array
	       (let ((counter 0))
		 (dolist (doc docs)
		   (incf counter)
		   (if (typep doc 'document)
		       (print-hash.3 (elements doc) stream :max max)
		       (format stream "~A," doc))
		   (when (> counter 100)
		     (progn
		       (format stream "[....](~A elements left)" (- (length docs) 100))
...
(defmethod print-object.3 ((document document) stream)
  (format stream "{ ~%")
  ...
  (if (slot-boundp document 'elements)
	...
	(print-hash.3 (elements document) stream :max 20))
      "no elements set..")
  (format stream "}~%"))
